...
RSpec.describe Anomalous::Loader do
  ...
  describe 'attributes' do
    subject do
      described_class.new 'spec/sample_data/data.mat', data_format: :matlab
    end

    it 'has :training_set that is 80% of non-anomalous examples' do
      expect(subject.training_set.rows).to eq 872
    end
    ...
    it 'has :cross_val_set that has 20% of non-anomalous examples' do
      non_anomalous = subject.cross_val_set.to_a.select { |e| e.last.zero? }

      expect(non_anomalous.size).to eq 218
    end

    it 'has :cross_val_set that has all anomalous examples' do
      anomalous = subject.cross_val_set.to_a.select { |e| e.last == 1 }

      expect(anomalous.size).to eq 10
    end
  end
end
